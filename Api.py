...
@app.route("/api/languages", methods=["GET"]) # Get all languages
def get_languages():
    return jsonify({"languages": languages})

@app.route("/api/languages/<int:lang_id>", methods=["GET"]) # Get language by id
...
@app.route("/api/languages/<string:lang_name>", methods=["GET"]) # Get language by name
def get_language_by_name(lang_name):
    language = next((lang for lang in languages if lang["name"].lower() == lang_name.lower()), None)
    if language:
        return jsonify(language)
    return jsonify({"error": "Language not found"}), 404
